...
if ($_POST['action'] == 'addCliente') {
  ...
  $query_insert = mysqli_query($conexion, "INSERT INTO cliente(dni, nombre, telefono, correo, direccion, usuario_id, fecha) VALUES ('$dni','$nomnre','$telefono','$correo','$direccion','$usuario_id', '$date')");
  if ($query_insert) {
...
if ($_POST['action'] == 'addProductoDetalle') {
  if (empty($_POST['producto']) || empty($_POST['cantidad'])){
    ...
    $query_detalle_temp = mysqli_query($conexion, "CALL add_detalle_temp ('$codproducto','$cantidad','$observacion','$mprecio','$token')");
    ...
    while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
      $nuevoprecio = round($data['precio_venta'] + $data['mprecio'], 2);
      $precioTotal = round($data['cantidad'] * $nuevoprecio, 2);
    ...
    <tr>
        ...
        <td class="textright">'.$total.'.00</td>
    </tr>';
...
if ($_POST['action'] == 'searchForDetalle') {
  ...
  if (empty($_POST['user'])){
    ...
    $query = mysqli_query($conexion, "SELECT tmp.correlativo, tmp.token_user,
      tmp.cantidad, tmp.precio_venta, p.codproducto, p.descripcion, tmp.observacion, tmp.mprecio
