...
if (!empty($_POST)) {
    ...
    if (empty($_POST['nombre']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
        ...
        if ($resul >= 1) {
            $alert = '<p class="error">La C.C ya existe</p>';
        } else {
            if ($dni == '') {
                $dni = 0;
            }
            $sql_update = mysqli_query($conexion, "UPDATE cliente SET dni = $dni, nombre = '$nombre' , telefono = '$telefono', correo = '$correo', direccion = '$direccion' WHERE idcliente = $idcliente");
