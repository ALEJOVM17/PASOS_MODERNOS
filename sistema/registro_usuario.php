...
if (!empty($_POST)) {
    $alert = "";
    if (empty($_POST['nombre']) || empty($_POST['correo']) || empty($_POST['telefono'])|| empty($_POST['usuario']) || empty($_POST['clave']) || empty($_POST['rol'])) {
        ...
        if ($result > 0) {
            ...
            $query_insert = mysqli_query($conexion, "INSERT INTO usuario(nombre,cedula,correo,usuario,clave,rol,telefono,estado) values ('$nombre', '$cedula','$email','$user', '$clave', '$rol', '$telefono',1)");
            if ($query_insert) {
                $alert = '<div class="alert alert-primary" role="alert">
                            Usuario registrado
                        </div>';
